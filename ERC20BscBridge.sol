...
contract ERC20BscBridge {

  uint chainId;
  
  uint public fee_percent = 5e5; // 0.5 %
  uint public minimum_fee_amount = 50; // 50 erc20
  ...
  function release(
    ...
    erc20.mint(address(this), request.amount);
    erc20.transfer(request.to, request.amount - fee_amount);
    if(penalty_amount > 0) {
      if(penalty_amount > fee_amount) {
        erc20.transfer(penalty_wallet, fee_amount);
        penalty_amount -= fee_amount;
      }
      else {
        erc20.transfer(penalty_wallet, penalty_amount);
        erc20.transfer(msg.sender, fee_amount - penalty_amount);
        penalty_amount -= penalty_amount;
      }
    }
    else {
      erc20.transfer(msg.sender, fee_amount);
    }
